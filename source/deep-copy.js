...
async function getUpdatedField(space, field, tag, depth) {
  ...
  if (
    ...
    const clonedEntry = await recursiveClone(
      space,
      field.sys.id,
      tag,
      depth + 1
...
async function getUpdatedFields(space, fields, tag, depth) {
  for (let fieldName in fields) {
    ...
    for (let lang in field) {
      ...
      fields[fieldName][lang] = await getUpdatedField(
        space,
        fields[fieldName][lang],
        tag,
        depth
      );
    }
...
async function recursiveClone(space, id, tag, depth) {
  ...
  const newEntry = await createNewEntry(space, entry.sys.contentType.sys.id, {
    fields: newFields,
  });
